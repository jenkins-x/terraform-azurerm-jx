...
| dns_enabled | Flag which sets whether a DNS zone is provisioned or not | `bool` | `false` | no |
...
| registry_resource_group | Name of resource group (to provision) in which to create registry. The script will create a random name if this is empty | `string` | `""` | no |
...
| tls\_email | Email used by Let's Encrypt. Required for TLS when parent\_domain is specified | `string` | `""` | no || velero\_namespace | Kubernetes namespace for Velero | `string` | `"velero"` | no |
